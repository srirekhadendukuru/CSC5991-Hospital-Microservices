...
def api():
    ...
    return requests.get(f"http://{GATEWAY_FQDN}/proxy/cleaning-staff-reader/cleaning-staff").json()

@app.route('/api/<name>', methods=['GET'])
def cleaning_api(name:str):
    # not ideal, but postback so we can use Docker's internal networking easier.
    return requests.get(f"http://{GATEWAY_FQDN}/proxy/cleaning-staff-reader/cleaning-staff/{name}").json()
