...
def api():
    ...
    return requests.get(f"http://{GATEWAY_FQDN}/proxy/billing-reader/billing").json()

@app.route('/api/<name>', methods=['GET'])
def room_api(name:str):
    # not ideal, but postback so we can use Docker's internal networking easier.
    return requests.get(f"http://{GATEWAY_FQDN}/proxy/billing-reader/billing/{name}").json()
